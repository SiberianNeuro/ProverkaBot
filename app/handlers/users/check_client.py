...
async def get_group_check_start(call: types.CallbackQuery, state: FSMContext, db_session: sessionmaker,
    ...
    if ticket.status_id == 1:
        ...
        logger.opt(lazy=True).log('CHECK', f'User {user.fullname} started checking client (ID: {callback_data.value})')


@router.callback_query(Checking.choice, CheckingCallback.filter(F.param == 'choice'), F.chat_type == 'private')
async def get_check_choice(call: types.CallbackQuery, state: FSMContext, db_session: sessionmaker, user: User,
    ...
    if call.message.date < datetime.now() - timedelta(hours=48):
        await call.message.reply(answer_text)
    else:
        with suppress(TelegramBadRequest):
            await call.message.edit_text(answer_text, reply_markup=None)
...
async def get_check_comment(msg: types.Message, state: FSMContext, db_session: sessionmaker, user: User,
    ...
    async with db_session() as session:
        try:
            current_ticket: Ticket = await session.get(Ticket, ticket_id)
            result = await session.execute(User.id).where(
